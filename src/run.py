...
from . import build_sheet, enrich, scrape_gbp
...
def main():
    ...
    if args.skip_scrape:
        ...
        with open(RAW_PATH, encoding="utf-8") as fh:
            places = json.load(fh)
        print(f"[run] reusing {len(places)} cached places")
    else:
        places = scrape_gbp.run(niches, args.city, args.state, cfg, args.max)

    if not places:
        raise SystemExit("No places returned. Check city/niche or Apify run.")
